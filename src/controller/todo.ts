...
import { TodoDocument } from '../entity/todo'
...
abstract class ITodoService {
  ...
  abstract getOne(id: string): Promise<TodoDocument | null>
...
class TodoController {
  ...
  async deleteOne(req: Request, res: Response) {
    ...
    try {
      ...
      if (!todoItem.author.equals(user._id)) {
        return res
          .status(401)
          .json({ message: 'only author can delete todo-item' })
      }
  ...
  async getAllByUserId(req: Request, res: Response) {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: 'некорректные значения page или limit' })
    }
    try {
      const todos = await this.todoService.getAllByUserId(
        user._id,
        Number(page),
        Number(limit)
      )
  ...
  async editOne(req: Request, res: Response) {
    ...
    try {
      ...
      if (!todoItem.author.equals(user._id)) {
        return res
          .status(401)
          .json({ message: 'only author can edit todo-item' })
      }
      const updatedTodoItem = await this.todoService.editOne(
        id,
        title,
        description
      )
